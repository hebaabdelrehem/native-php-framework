			...
			while ( $file = readdir ( $handle ) ) {
				if (is_file ( $from_folder . $file )) {
					if (strpos ( $from_folder . $file, '.plugin.php' )) {
                        ...
                        preg_match ( '|Icon:(.*)$|mi', $plugin_data, $icon );
                        preg_match ( '|folder:(.*)$|mi', $plugin_data, $folder );
						
						foreach ( array ('name', 'uri', 'version', 'description', 'author_name', 'author_uri', 'icon', 'folder' ) as $field ) {
							if (! empty ( ${$field} ))
								${$field} = trim ( ${$field} [1] );
							else
								${$field} = '';
						}
						$plugin_data = array ('filename' => $file, 'Name' => $name, 'Title' => $name, 'PluginURI' => $uri, 'Description' => $description, 'Author' => $author_name, 
                        'AuthorURI' => $author_uri, 'Version' => $version, 'Icon' => $icon, 'folder' => $folder );
